...
const VOYAGER_STYLE='https://tiles.openfreemap.org/styles/liberty';
...
function locateUser(showMessage=true){if(!navigator.geolocation){toast('GPS ამ ბრაუზერში მიუწვდომელია');return}navigator.geolocation.getCurrentPosition(position=>{userLocation={lat:position.coords.latitude,lng:position.coords.longitude};if(!userMarker){const el=document.createElement('div');el.className='user-location vehicle-arrow';userMarker=new maplibregl.Marker({element:el}).setLngLat([userLocation.lng,userLocation.lat]).addTo(map)}else userMarker.setLngLat([userLocation.lng,userLocation.lat]);map.easeTo({center:[userLocation.lng,userLocation.lat],zoom:17,pitch:is3d?55:0});renderList();if(showMessage)toast('მდებარეობა განახლებულია')},()=>toast('მდებარეობის მიღება ვერ მოხერხდა'),{enableHighAccuracy:true,timeout:10000,maximumAge:30000})}
...
function startSpeedTracking(){
  ...
  navigator.geolocation.watchPosition(position=>{
    ...
    if(!userMarker){const el=document.createElement('div');el.className='user-location vehicle-arrow';userMarker=new maplibregl.Marker({element:el}).setLngLat([point.lng,point.lat]).addTo(map)}else userMarker.setLngLat([point.lng,point.lat]);
...
async function buildRoute(){if(!selected)return;if(!userLocation){toast('ჯერ ჩართე მიმდინარე მდებარეობა');locateUser(false);return}$('routeButton').textContent='იგეგმება…';try{const params=new URLSearchParams({from:`${userLocation.lat},${userLocation.lng}`,to:`${selected.lat},${selected.lng}`});const response=await fetch(`${SUPABASE_URL}/functions/v1/route?${params}`);if(!response.ok)throw new Error('No route');const route=await response.json(),source=map.getSource('active-route'),minutes=Math.max(1,Math.round(route.duration/60)),arrival=new Date(route.arrivalTime||Date.now()+route.duration*1000);source.setData({type:'Feature',geometry:route.geometry,properties:{}});routeActive=true;document.body.classList.add('navigation-active');$('routeDestination').textContent=selected.address||selected.name||'დანიშნულების ადგილი';$('routeDistance').textContent=km(route.distance);$('routeDuration').textContent=minutes>=60?`${Math.floor(minutes/60)}სთ ${minutes%60}წთ`:`${minutes} წთ`;$('routeArrival').textContent=arrival.toLocaleTimeString('ka-GE',{hour:'2-digit',minute:'2-digit',hour12:false});$('maneuverDistance').textContent=km(Math.min(route.distance,500));$('maneuverInstruction').textContent='გააგრძელეთ მონიშნულ მარშრუტზე';$('maneuverCard').hidden=false;$('routeCard').hidden=false;$('chargerDetail').hidden=true;const bounds=new maplibregl.LngLatBounds();route.geometry.coordinates.forEach(x=>bounds.extend(x));map.fitBounds(bounds,{padding:{top:150,right:90,bottom:100,left:innerWidth>850?390:90},maxZoom:15,duration:1000})}catch(error){toast('მარშრუტის აგება ვერ მოხერხდა')}finally{$('routeButton').textContent='მარშრუტი'}}
