...
function restoreMapLayers(){if(darkModeEnabled&&map.getLayer('background'))map.setPaintProperty('background','background-color','#152541');if(!map.getSource('active-route'))map.addSource('active-route',{type:'geojson',data:activeRouteFeature});if(!map.getLayer('active-route-line'))map.addLayer({id:'active-route-line',type:'line',source:'active-route',layout:{'line-cap':'round','line-join':'round'},paint:{'line-color':'#7428f5','line-width':7,'line-opacity':.95}});add3dLayers();if(naprEnabled)map.once('idle',updateNaprOverlay)}
